...
def decompress(text, algorithm="zlib"):
    if algorithm == "zlib":
        r = zlib.decompress(text)
    if algorithm == "zstd":
        r = zstd.decompress(text)
    return r.decode("utf8")


def parse_args():
    parser = argparse.ArgumentParser(
        description="LLM Distiller with Bloom filter and SQLite storage."
    )
    parser.add_argument(
        "--prompt", default=None, help="Root word or prompt to distill."
    )
    parser.add_argument(
        "--model",
        default="distilgpt2",
    ...
    parser.add_argument(
        "--db",
        default="words/data.db",
        help="Path to SQLite database (default: words/data.db).",
    )
    parser.add_argument(
        "--max-depth", type=int, default=10, help="Max recursion depth (default: 10)."
    )
    parser.add_argument(
        "--max-tokens", type=int, default=1024, help="Max tokens (default: 1024)."
    )
    parser.add_argument(
        "--compression-level",
        type=int,
    ...
    parser.add_argument(
        ...
        type=int,
        default=26,
        help="Bloom filter size (default: 100,000,000).",
    )
    parser.add_argument(
        "--bloom-hash-count",
        type=int,
        default=6,
        help="Bloom filter hash count (default: 6).",
    )
    parser.add_argument(
        "--max-ngrams", type=int, default=10, help="Max ngrams (default: 10)."
    )
    parser.add_argument(
    ...
    parser.add_argument(
        "--retrieve-to-bloom",
        action="store_true",
        help="Retrieve words from the database to the Bloom filter.",
    )
    ...
    parser.add_argument(
        "--secrets-file", default=None, help="Specify the secrets json file."
    )
    parser.add_argument(
        "--load-prompts-from-file", default=None, help="Specify the prompts file file."
    )
    parser.add_argument(
        "--api-hf-provider",
        default=None,
        help="Specify the hugging face inference provider",
    )
    parser.add_argument(
        "--compression-algo", default="zlib", help="Specify the compresion algo to use."
    )
    parser.add_argument(
        "--prompt-prefixes", nargs="+", help="List of strings with spaces allowed"
    )
    parser.add_argument(
        "--batch-size",
        type=int,
    ...
    parser.add_argument(
        ...
        action="store_true",
        default=False,
        help="Remove the prompt from generation.",
    )
    parser.add_argument(
        "--ngram-mode",
        action="store_true",
        default=False,
        help="ngram mode from generation.",
    )
    parser.add_argument(
        "--min-tfidf-score",
        type=float,
        default=0.2,
        help="Specify the min_tfidf_score.",
    )
    parser.add_argument(
        "--save-to-textfile",
        default=None,
        help="Specify a text file to save generated text.",
    )
    ...
    parser.add_argument(
        ...
        action="store_true",
        default=None,
        help="Randomize prompts when read from file.",
    )
    parser.add_argument(
        "--randomize-model-retry",
        action="store_true",
        default=None,
        help="Randomize model to retry.",
    )
    parser.add_argument(
        "--randomize-remote-endpoint",
        action="store_true",
        default=None,
        help="Randomize remote endpoint.",
    )
    parser.add_argument(
        "--strip-think-tag-form-prompt",
        action="store_true",
        default=None,
        help="Strip the <think> and </think> tags from prompts.",
    )
    parser.add_argument(
        "--exp-backoff",
        action="store_true",
        default=False,
        help="Set exponential backoff.",
    )
    parser.add_argument(
        "--stream", action="store_true", default=False, help="Set stream."
    )
...
def backup_file(filepath, path):
    ...
    backups = sorted(
        dst_dir.glob(pattern), key=lambda p: p.stat().st_mtime, reverse=True
    )
